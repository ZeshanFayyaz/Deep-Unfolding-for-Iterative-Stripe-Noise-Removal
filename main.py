...
for i in range(3):
    ...
    for j in range(10):
        for images, label in training_data:
            ...
            X.append(Degraded[0] / 255.0) #X is the Dirty Training Images
            z.append(images / 255.0) #z is the Clean Training Images [Target] 

        for images, label in validation_data:
            Degraded = Degrade(images, 25)
            X_validation.append(Degraded[0] / 255.0) #X_validation is the Dirty Validation Images
    ...
    #Place them into X_train and z_train so I can test them later
    X_train = X[0:2]
    z_train = z[0:2]
...
#Recall, X_train is a subset of training images with random noise (we defined this in the network itself) 
