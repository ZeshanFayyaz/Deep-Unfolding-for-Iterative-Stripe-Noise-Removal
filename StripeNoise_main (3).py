...
import numpy as np
from PIL import Image
import random, itertools, operator, math
import tensorflow as tf
from tensorflow import keras
from tensorflow.keras import models
from tensorflow.keras import layers
from tensorflow.keras.models import Sequential, Model
from tensorflow.keras.layers import Input, Dense, Dropout, Activation, Flatten, Conv2D, MaxPooling2D, BatchNormalization, GRU, Bidirectional, TimeDistributed
...
DATADIR = "C:\\Stripe\\Stripe\\Linnaeus_5_256X256\\test_set\\train_gray\\"
TESTDIR = "C:\\Stripe\\Stripe\\Linnaeus_5_256X256\\test_set\\test_gray\\"
...
print("Using 90% Train and 10% Validation...")
numtosplit =int(0.9*(len(training_validation_data)))
...
for i in range(num_epochs):
    ...
    for j in range(5):
        ...
        print("Done Adding Noise Instance: " + str(j)) 
...
def NoiseRemoval(epoch_input, b_size, Num_GRU, Size_GRU):
    
    inputs = Input(shape = (256,256))
    inputs_t = tf.transpose(inputs, perm=[0, 2, 1])
    
    for i in range(Num_GRU-1): 
        inputs_t = (Bidirectional(GRU(Size_GRU, return_sequences = True)))(inputs_t)
        
    x = Bidirectional(GRU(Size_GRU, return_sequences = True))(inputs_t)
...
PSNR = compare_psnr(clean_testing_data[0], model_testing_data[0])
print("PSNR of Original Degraded Image: " + str(PSNR))
PSNR2 = compare_psnr(clean_testing_data[0], output_test[0])
...
SSIM = compare_ssim(clean_testing_data[0], model_testing_data[0])
print("SSIM of Original Degraded Image in Reference to Ground Truth: " + str(SSIM))
SSIM2 = compare_ssim(clean_testing_data[0], output_test[0])
...
#Plot 3: Predicted Clean Image 
ax3.imshow(output_train[0], cmap = "gray")
ax3.title.set_text('Predicted Image')
...
PSNR_train = compare_psnr(z[0], X[0])
print("PSNR of Degraded Training Image: " + str(PSNR_train))
PSNR2_train = compare_psnr(z[0], output_train[0])
